...
void Maze::loadMenuImage(char* FileName){
   menuTex = TextureLoader(FileName);
}
...
GridLoc Maze::GetChestLoc() {// get chest pos
   GridLoc val;
...
void Maze::drawArrows() { // draw arrow
    if(liveSetOfArrws)
    {
...
void Maze::drawChest() {  // draw chest
    if(liveChest){
        glColor3f(1.0,1.0,1.0);
...
loc Maze::converter(int x, int y) {   // convert pos
    loc val;
